...
async def get_audit_logs(
    ...
    query = db.query(AuditLog).filter(
        AuditLog.tenant_id == current_user.tenant_id, AuditLog.created_at >= since
    )
    ...
    for log in logs:
        actor = (
            db.query(User).filter(User.id == log.actor_id).first()
            if log.actor_id
            else None
        )
        result.append(
...
async def get_audit_actions(
    current_user: User = Depends(get_hr_admin), db: Session = Depends(get_db)
):
...
async def get_entity_types(
    ...
    types = (
        ...
        .filter(
            AuditLog.tenant_id == current_user.tenant_id, AuditLog.entity_type != None
        )
        .distinct()
        .all()
    )
