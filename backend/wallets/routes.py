...
from models import AuditLog, User, Wallet, WalletLedger
from sqlalchemy.orm import Session
from wallets.schemas import (
    BulkPointsAllocationRequest,
    PointsAdjustmentRequest,
    PointsAllocationRequest,
...
async def get_my_wallet_ledger(
    ...
    ledger = (
        query.order_by(WalletLedger.created_at.desc()).offset(skip).limit(limit).all()
    )
    return ledger
...
async def get_user_wallet_ledger(
    ...
    wallet = (
        db.query(Wallet)
        .filter(Wallet.user_id == user_id, Wallet.tenant_id == current_user.tenant_id)
        .first()
    )
    if not wallet:
        raise HTTPException(status_code=404, detail="Wallet not found")
    ...
    ledger = (
        ...
        .filter(WalletLedger.wallet_id == wallet.id)
...
async def allocate_points(
    ...
    audit = AuditLog(
        ...
        new_values={
            ...
            "points_added": str(allocation.points),
        },
    )
    db.add(audit)
...
async def bulk_allocate_points(
    ...
    for allocation in allocation_data.allocations:
        ...
        wallet.lifetime_earned = (
            Decimal(str(wallet.lifetime_earned)) + allocation.points
        )
...
async def adjust_wallet_balance(
    ...
    if adjustment.adjustment_type == "credit":
        ...
        wallet.lifetime_earned = (
            Decimal(str(wallet.lifetime_earned)) + adjustment.points
        )
    elif adjustment.adjustment_type == "debit":
        if wallet.balance < adjustment.points:
            raise HTTPException(
                status_code=400, detail="Insufficient balance for debit adjustment"
            )
        wallet.balance = Decimal(str(wallet.balance)) - adjustment.points
    ...
    audit = AuditLog(
        ...
        new_values={
            "balance": str(wallet.balance),
            "adjustment": str(adjustment.points),
